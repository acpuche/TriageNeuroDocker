...
INPUTDIR="$1" #load /input directory, DICOM files
[ "$1" = "" ] && Usage

BASEDIR="$2" #load /output directory to store the files generated by the processing and prediction steps.
[ "$2" = "" ] && Usage
# Global vars def
#dcm2nii_path=/Applications/mricro
mni_brain=${FSLDIR}/data/standard/MNI152_T1_2mm_brain

files=($(ls ${INPUTDIR}/*.dcm))
dcm2json ${files[0]} ${BASEDIR}/description.json #quitar ${baseDIR} si se está trabajando sobre el directorio de las imagenes

mcverter -f fsl -d -n -F +SeriesDescription -o ${BASEDIR} ${INPUTDIR}
